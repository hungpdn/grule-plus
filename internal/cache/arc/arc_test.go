...
func TestEvictionPolicy(t *testing.T) {
	...
	// Access key1 to make it frequent
	cache.Get("key1")
...
func TestARCAdaptation(t *testing.T) {
	...
	cache.Set("item1", "value1", 0)
	cache.Set("item2", "value2", 0)
	cache.Set("item3", "value3", 0)
	cache.Set("item4", "value4", 0)

	// Access item1 multiple times to make it "frequent"
	for i := 0; i < 5; i++ {
		cache.Get("item1")
	}

	// Add more items to trigger eviction
	cache.Set("item5", "value5", 0)
	cache.Set("item6", "value6", 0)

	// item1 should still be in cache due to frequent access
	if !cache.Has("item1") {
		t.Error("expected frequently accessed item1 to remain in cache")
	}
}
